...
from .data_utils import get_heterogeneous_dataloaders, get_public_dataset_tensors
from . import config
...
def run():
    ...
    logger = FLLogger(output_dir=config.OUTPUT_DIR, experiment_name="FedMD")
    ...
    print(f"Logits Matching Rounds: {config.N_LOGITS_MATCHING_ROUND}")
    print(f"Private Training Rounds: {config.N_PRIVATE_TRAINING_ROUND}")
    print(f"Clients: {config.NUM_CLIENTS}")
    print(f"Label Heterogeneity: {config.LABEL_HETEROGENEITY}")
    print(f"Public Alignment Size: {config.N_ALIGNMENT}")
    ...
    public_dataset = get_public_dataset_tensors(public_loaders[0])
    ...
    for round_num in range(1, config.NUM_ROUNDS + 1):
        ...
        alignment_batches, _, _ = server.generate_alignment_data(
            public_dataset, config.N_ALIGNMENT, batch_size=config.BATCH_SIZE
        )
